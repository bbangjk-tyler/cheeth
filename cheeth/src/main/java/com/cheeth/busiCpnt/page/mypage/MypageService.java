...
public class MypageService extends AbstractService {
	...
	public Map<String, Object> getData01(Map<String, Object> parameter) throws Exception {
		...
		Integer page = 0;
		if (!ObjectUtils.isEmpty(parameter.get("PAGE"))
				&& parameter.get("PAGE").toString().chars().allMatch(Character::isDigit)) {
	...
	public Map<?, ?> getProfile(Map<String, Object> parameter) throws Exception {
		...
		String targetUserId = ObjectUtils.isEmpty(parameter.get("TARGET_USER_ID")) ? ""
				: parameter.get("TARGET_USER_ID").toString();
	...
	public Map<String, Object> save02(Map<String, Object> parameter) throws Exception {
		...
		String fileList = ObjectUtils.isEmpty(parameter.get("fileList")) ? ""
				: ParameterUtil.reverseCleanXSS(parameter.get("fileList").toString());
		if (cnt04 > 0 && cnt05 == 0 && !ObjectUtils.isEmpty(wrNo) && !ObjectUtils.isEmpty(multipartFileList)
				&& !ObjectUtils.isEmpty(fileList)) {
...
//    	delete("delete01", parameter);
	...
	@Transactional(propagation = Propagation.REQUIRED)
	public Map<String, Object> save07(HttpServletRequest request, Map<String, Object> parameter) throws Exception {
		...
		if(parameter.get("compfile") == null || parameter.get("compfile").toString() == "") {
		}else {
			parameter.put("COMP_FILE_CD", parameter.get("compfile").toString());
		}
		if(parameter.get("licensecd") == null || parameter.get("licensecd").toString() == "") {
		}else {
			parameter.put("LICENSE_FILE_CD", parameter.get("licensecd").toString());
		}
...
//			fileUtil.saveMultiPartFileBack(compFiles.get(((int) fileInfo.get("FILE_NO")) - 1), fileInfo,
...
//			fileUtil.saveMultiPartFileBack(LicenseFiles.get(((int) fileInfo2.get("FILE_NO")) - 1), fileInfo2,
//					uploadDir + File.separator);
...
//		}
	...
	public Map<String, Object> save08(HttpServletRequest request, Map<String, Object> parameter) throws Exception {
		...
		String userNickName = ObjectUtils.isEmpty(parameter.get("USER_NICK_NAME")) ? ""
				: parameter.get("USER_NICK_NAME").toString().trim();
	...
	public Map<String, Object> save09(Map<String, Object> parameter) throws Exception {
		...
		String profileFileCd = ObjectUtils.isEmpty(parameter.get("PROFILE_FILE_CD")) ? ""
				: parameter.get("PROFILE_FILE_CD").toString();
		String imageFileCd = ObjectUtils.isEmpty(parameter.get("IMAGE_FILE_CD")) ? ""
				: parameter.get("IMAGE_FILE_CD").toString();
		...
		if (cnt == 0) {
			...
			if (!ObjectUtils.isEmpty(profileFileCd))
				update("update09", parameter); // 프로필 파일
			if (!ObjectUtils.isEmpty(imageFileCd))
				update("update10", parameter); // 이미지 파일
		}
