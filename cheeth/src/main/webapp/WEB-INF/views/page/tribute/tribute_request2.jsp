...
/* 	var obj = {
	...
	function rewrite(){
		...
		cardArr2.map(m => {
			$('#PANT_NM').val(cardArr[0]['PANT_NM']);
			if(isNotEmpty(m['SUPP_NM_1'])) {
				console.log("m['SUPP_NM_1'] " + m['SUPP_NM_1']);
				var prostheticsTypo ='보철 종류를 선택해 주세요.';
				prostheticsTypo = m['SUPP_NM_1'];
				if(typeof m['SUPP_NM_2'] == "undefined" || m['SUPP_NM_2'] == null || m['SUPP_NM_2'] == ""){}else{
					prostheticsTypo += " - " + m['SUPP_NM_2'];
				}
				if(typeof m['SUPP_NM_3'] == "undefined" || m['SUPP_NM_3'] == null || m['SUPP_NM_3'] == ""){}else{
					prostheticsTypo += " - " +m['SUPP_NM_3'];
				}
				if(typeof m['SUPP_NM_4'] == "undefined" || m['SUPP_NM_4'] == null || m['SUPP_NM_4'] == ""){}else{
					prostheticsTypo += " - " +m['SUPP_NM_4'];
				}
			}

			var del = "{";
			var end = m['TRIBUTE_DTL'].length - 1;
			var index = 0;
			...
			del += "}";
		    var currCardObj2 = { 'TAB_NO' : m.TAB_NO,
                    'EXCEPTION_BRIDGE' : m.EXCEPTION_BRIDGE,
                    'SUPP_INFO' : prostheticsTypo,
                    'PANT_NM' : m.PANT_NM,
                    'FILE_CD' : m.FILE_CD,
                    'BRIDGES' : m.BRIDGES,
...
/* 			$('.bridge').each(function() {
	...
	function saveCard3(tabIndex) {
		tabIndex = tabIndex - 1;
		console.log("헬로우 " + tabIndex);
		currCardObj = {...cardArr[tabIndex]};
		for(const [key, value] of Object.entries(currCardObj)) {
			  console.log(currCardObj[key]);
		} 
		// 치식
		$('.teeth_model_tooth_img').each(function() {
			var $this = $(this);
...
/* 		  alert(
	...
	function setCard() {
		...
		console.log("setCard1 curtabnum" + curtabnum);

		console.log("setCard1 justonebool" + justonebool);

		console.log("setCard1 toporbottom" + toporbottom);

		console.log("setCard1 CategoryChoiceBool" + CategoryChoiceBool);
	...
	function fnSave() {
		...
		if(confirm(getI8nMsg("alert.confirm.save"))) { //저장하시겠습니까?
			...
			/* $.ajax({
				...
				url: '/' + API + 'tribute/setting',

				type: 'POST',

				data: JSON.stringify({ cardsetting : justoneboolArray}),
  ...
  <div class="tribute_request_card">
		...
		<div class="tribute_request_card_chip_container">
		  ...
		  <div class="tribute_request_card_chip_selected card_chip">
		    ...
		    <p class="tribute_request_card_chip_selected_typo card_title" id="1" onclick="setCard(this);">의뢰서 1</p>
