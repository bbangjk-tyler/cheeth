				...
				reader.onload = (event) => {
					...
					/* $('.qna_pic_attatchment_sub_pic_upload').each(function(index, item) {
						if($(item).css('background-image') == 'none') {
							...
							$(item).css('background-image', 'url(' + imageSrc + ')');
							$(item).append('<img class="qna_pic_attatchment_remove_button" src="/public/assets/images/dialog_close_button.svg" onclick="removeImage(this);"/>');
							uploadImgArr[index] = { 'FILE' : file, 'IMG_SRC' : imageSrc };
							return false;
						}
					}); */
				};
...
<div class="qna_header">
	<p class="qna_header_typo">1:1 게시판</p>
...
<div class="qna_body">
  ...
  <div class="qna_main_container">
    ...
    <div class="qna_list_container">
      <div class="qna_list_data_type_container">
        <div class="qna_list_data_type_order">
          <p class="qna_list_data_type_order_typo">NO</p>
        </div>
        <div class="qna_list_data_type_title">
          <p class="qna_list_data_type_title_typo">제목</p>
        </div>
        <div class="qna_list_data_type_date">
          <p class="qna_list_data_type_typo">등록일</p>
        </div>
      </div>
      <c:forEach items="${LIST}" var="item">
      	<div class="list_divider"></div>
	      <div class="qna_list">
	        <div class="qna_list_order">
	          <p class="qna_list_typo">${item.RN}</p>
	        </div>
	        <a href="/${api}/board/view?BOARD_TYPE=${item.BOARD_TYPE}&BOARD_SEQ=${item.BOARD_SEQ}" class="qna_list_title">
	          <div class="qna_list_reply ${empty item.UP_BOARD_SEQ ? 'hidden' : ''}">
	            <p class="qna_list_reply_typo">Re</p>
	          </div>
	          <p class="qna_list_typo">${item.BOARD_TITLE}</p>
	        </a>
	        <div class="qna_list_date">
	          <div class="qna_list_typo">${item.CREATE_DATE}</div>
	        </div>
	      </div>
      </c:forEach>
      <c:if test="${empty LIST}">
		  	...
		  	<div class="qna_list">
		    	<div class="qna_list_order">
		        <p class="qna_list_typo"></p>
		      </div>
		      <div class="qna_list_title" style="padding-left: 285px;">
			      <p class="qna_list_typo">등록된 게시글이 없습니다.</p>
		      </div>
		      <div class="qna_list_date">
		        <div class="qna_list_typo"></div>
		      </div>
		    </div>
		  </c:if>
      <div class="list_divider"></div>
